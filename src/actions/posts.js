...
export const getPosts = () => (dispatch, getState) => {
  ...
  axiosInstance
    .get("/posts/", reduxTokenConfig(getState))
    .then((res) => {
      dispatch({
        type: GOT_POSTS,
        payload: res.data,
    ...
    .catch((err) => {
      if (err.response.status === 401) {
        // If user not authenticated with Spotify
...
export const getUserPosts = (username, setPosts, token) => {
  ...
  axiosInstance
    .get(`/posts/user-posts/${username}`, formatHeader(token))
    .then((res) => {
      setPosts(res.data);
...
export const deletePost = (id) => (dispatch, getState) => {
  axiosInstance
    .delete(`/posts/${id}/`, reduxTokenConfig(getState))
    .then((res) => {
      dispatch({
        type: DELETE_POST,
        payload: id,
...
export const addPost = (post) => (dispatch, getState) => {
  axiosInstance
    .post("/posts/", post, reduxTokenConfig(getState))
    .then((res) => {
      dispatch({
        type: ADD_POST,
        payload: res.data,
