...
function FullScreenPopupLayer(props) {
  ...
  const [linkClasses, setLinkClasses] = useState(
    initArray(children.length, false),
  );
  ...
  const generateListItems = () => {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      tags.push(
        <li
          key={child.props.uniqueKeyString}
          onClick={onClick}
          className={
            "my-1 link-box" + " " + (linkClasses[i] ? "text-final-state" : "")
          }
        >
          {child}
        </li>,
  ...
  const popupLayer = (
    <div
      className={
        "popup-layer" +
        " " +
        (transitionInFinalState ? "popup-layer-enabled" : "")
      }
    >
      <div className={"link-container"}>
        <div>
          <ul className={"nav flex-column"}>{generateListItems()}</ul>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <div
        className={
          "menu-button" + " " + (transitionInFinalState ? "change" : "")
        }
        onClick={onClick}
      >
