...
app.get("*/*.js", (req, res) => {
    ...
    res.sendFile(
        path.join(
            __dirname + "/../dist/static/js/" + urlParts[urlParts.length - 1]
        )
    );
});
...
app.get("*/*.css", (req, res) => {
    ...
    res.sendFile(
        path.join(
            __dirname + "/../dist/static/css/" + urlParts[urlParts.length - 1]
        )
    );
});
...
app.get("*/*.(jpg|svg|png|woff|woff2)", (req, res) => {
    ...
    res.sendFile(
        path.join(
            __dirname + "/../dist/static/media/" + urlParts[urlParts.length - 1]
        )
    );
});
